...
from ICD_10_ANALYSIS.Country_Map import get_code, get_country
from ICD_10_ANALYSIS.PopStat_Calculation import POP_STAT_CALCULATION_FOR_OTHER_DISEASES
from ICD_10_ANALYSIS.Death_Data_Processor import DEATH_DATA_PROCESSOR_BY_DISEASE
from ANALYSIS.Population_Data_For_Date import POPULATION_DATA_FOR_DATE

death_data_path = os.path.join(main_dir, "ICD-10-DATA","Death_Data","Death_Data_By_Disease_2021.csv")
...
SAVING_PATH_PNG = os.path.join(main_dir, "ICD-10-RESULTS", "CORRELATION_WITH_OTHER_DISEASES", "POPSTAT")
SAVING_PATH_PNG_HDI = os.path.join(main_dir, "ICD-10-RESULTS", "CORRELATION_WITH_OTHER_DISEASES", "OTHER_METRICS", "HDI")
SAVING_PATH_PNG_MEDIAN_AGE = os.path.join(main_dir, "ICD-10-RESULTS", "CORRELATION_WITH_OTHER_DISEASES", "OTHER_METRICS", "MEDIAN_AGE")
SAVING_PATH_PNG_GDP_PER_CAPITA = os.path.join(main_dir, "ICD-10-RESULTS", "CORRELATION_WITH_OTHER_DISEASES", "OTHER_METRICS", "GDP_PER_CAPITA")
SAVING_PATH_PNG_POPULATION_DENSITY = os.path.join(main_dir, "ICD-10-RESULTS", "CORRELATION_WITH_OTHER_DISEASES", "OTHER_METRICS", "POPULATION_DENSITY")
SAVING_PATH_CSV = os.path.join(main_dir, "ICD-10-RESULTS", "CORRELATION_DATA_FOR_OTHER_DISEASES")
SAVING_PATH_PNG_SDI = os.path.join(main_dir, "ICD-10-RESULTS", "CORRELATION_WITH_OTHER_DISEASES", "OTHER_METRICS", "SDI")
SAVING_PATH_PNG_GINI = os.path.join(main_dir, "ICD-10-RESULTS", "CORRELATION_WITH_OTHER_DISEASES", "OTHER_METRICS", "GINI")
SAVING_PATH_PNG_UHCI = os.path.join(main_dir, "ICD-10-RESULTS", "CORRELATION_WITH_OTHER_DISEASES", "OTHER_METRICS", "UHCI")
SAVING_PATH_PNG_LIFE_EXPECTANCY = os.path.join(main_dir, "ICD-10-RESULTS", "CORRELATION_WITH_OTHER_DISEASES", "OTHER_METRICS", "LIFE_EXPECTANCY")

POPSTAT_DISEASE_DATA_DIR = os.path.join(main_dir, "ICD-10-RESULTS", "POPSTAT_OTHER_DISEASES")

class MORTALITY_DATA:
    def __init__(self, year):
        self.year = year
        DEATH_DATA_PROCESSOR_BY_DISEASE(year)
    ...
    def filter_death_data(self, disease):
        self.data = self.DEATH_DATA[self.DEATH_DATA['Cause Code'] == disease]
    ...
    def create_death_data_per_disease(self, country):
        ...
        if filtered_data.empty:
            return None
        total_deaths = filtered_data['Deaths per Million'].values[0]
        return total_deaths
    ...
    def ANALYZER(self):
        causes = self.DEATH_DATA['Cause Code'].unique()
